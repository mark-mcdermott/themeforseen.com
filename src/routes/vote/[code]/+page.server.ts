...
import { createDb, abTests, customPalettes, withRetry } from '$lib/server/db';
...
export const load: PageServerLoad = async ({ params, platform }) => {
	...
	// Find the test by share code (with retry for cold starts)
	const test = await withRetry(() =>
		db.select().from(abTests).where(eq(abTests.shareCode, params.code)).limit(1)
	);
	...
	const customPaletteA = await withRetry(() =>
		db
	...
	const customPaletteB = await withRetry(() =>
		db
			.select({ light: customPalettes.light, dark: customPalettes.dark })
			.from(customPalettes)
